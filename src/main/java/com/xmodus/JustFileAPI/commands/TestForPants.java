...
public class TestForPants implements Command<CommandSource> {

    private static final TestForPants CMD = new TestForPants();

    public static final Logger LOGGER = LogManager.getLogger();

    // TODO: modify so we can run a named command w/ a named results file
    public static ArgumentBuilder<CommandSource, ?> register(CommandDispatcher<CommandSource> dispatcher) {
        return Commands.literal("TestForPants")
    ...
    public int run(CommandContext<CommandSource> context) throws CommandSyntaxException {
        ...
        // test for "Co-op Pants" then TP to x, y, z
        String item_name = Config.IG_HAS_PANTS_ITEM.get();
        // source position
        int sx = Config.IG_HAS_PANTS_SX.get();
        int sy = Config.IG_HAS_PANTS_SY.get();
        int sz = Config.IG_HAS_PANTS_SZ.get();
        // dest position
        int dx = Config.IG_HAS_PANTS_DX.get();
        int dy = Config.IG_HAS_PANTS_DY.get();
        int dz = Config.IG_HAS_PANTS_DZ.get();

        LOGGER.info("[TestForPants] testing for "
                + item_name
                + " at:"
                + " " + sx
                + " " + sy
