...
public class TestForArmor implements Command<CommandSource> {

    private static final TestForArmor CMD = new TestForArmor();

    public static final Logger LOGGER = LogManager.getLogger();

    // TODO: modify so we can run a named command w/ a named results file
    public static ArgumentBuilder<CommandSource, ?> register(CommandDispatcher<CommandSource> dispatcher) {
        return Commands.literal("TestForArmor")
    ...
    public int run(CommandContext<CommandSource> context) throws CommandSyntaxException {
        ...
        // test for "Co-op Armor" then TP to x, y, z
        String item_name = Config.IG_HAS_ARMOR_ITEM.get();
        // source position
        int sx = Config.IG_HAS_ARMOR_SX.get();
        int sy = Config.IG_HAS_ARMOR_SY.get();
        int sz = Config.IG_HAS_ARMOR_SZ.get();
        // dest position
        int dx = Config.IG_HAS_ARMOR_DX.get();
        int dy = Config.IG_HAS_ARMOR_DY.get();
        int dz = Config.IG_HAS_ARMOR_DZ.get();

        LOGGER.info("[TestForArmor] testing for "
                + item_name
                + " at:"
                + " " + sx
                + " " + sy
