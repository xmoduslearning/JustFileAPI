...
public class TestForBoots implements Command<CommandSource> {

    private static final TestForBoots CMD = new TestForBoots();

    public static final Logger LOGGER = LogManager.getLogger();

    // TODO: modify so we can run a named command w/ a named results file
    public static ArgumentBuilder<CommandSource, ?> register(CommandDispatcher<CommandSource> dispatcher) {
        return Commands.literal("TestForBoots")
    ...
    public int run(CommandContext<CommandSource> context) throws CommandSyntaxException {
        ...
        // test for "Co-op Boots" then TP to x, y, z
        String item_name = Config.IG_HAS_BOOTS_ITEM.get();
        // source position
        int sx = Config.IG_HAS_BOOTS_SX.get();
        int sy = Config.IG_HAS_BOOTS_SY.get();
        int sz = Config.IG_HAS_BOOTS_SZ.get();
        // dest position
        int dx = Config.IG_HAS_BOOTS_DX.get();
        int dy = Config.IG_HAS_BOOTS_DY.get();
        int dz = Config.IG_HAS_BOOTS_DZ.get();

        LOGGER.info("[TestForBoots] testing for "
                + item_name
                + " at:"
                + " " + sx
                + " " + sy
