...
public class TestForSword implements Command<CommandSource> {

    private static final TestForSword CMD = new TestForSword();

    public static final Logger LOGGER = LogManager.getLogger();

    // TODO: modify so we can run a named command w/ a named results file
    public static ArgumentBuilder<CommandSource, ?> register(CommandDispatcher<CommandSource> dispatcher) {
        return Commands.literal("TestForSword")
    ...
    public int run(CommandContext<CommandSource> context) throws CommandSyntaxException {
        ...
        // test for "Co-op Sword" then TP to x, y, z
        String item_name = Config.IG_HAS_SWORD_ITEM.get();
        // source position
        int sx = Config.IG_HAS_SWORD_SX.get();
        int sy = Config.IG_HAS_SWORD_SY.get();
        int sz = Config.IG_HAS_SWORD_SZ.get();
        // dest position
        int dx = Config.IG_HAS_SWORD_DX.get();
        int dy = Config.IG_HAS_SWORD_DY.get();
        int dz = Config.IG_HAS_SWORD_DZ.get();

        LOGGER.info("[TestForSword] testing for "
                + item_name
                + " at:"
                + " " + sx
                + " " + sy
